...
if (-not $IsAdmin) {
    ...
    $InstallUrl = "https://raw.githubusercontent.com/lmalma/claude-desktop-rtl-patch/selfhost/install.ps1"
    if ($Auto) { $env:CLAUDE_RTL_AUTO = '1' }
...
if (-not (Test-Path $pubkeyPinFile)) {
    ...
    Write-Host "  irm https://raw.githubusercontent.com/lmalma/claude-desktop-rtl-patch/selfhost/install.ps1 | iex" -ForegroundColor Cyan
    Pause-ThenExit 1
}
